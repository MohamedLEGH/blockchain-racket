  ...
  (test-suite
   ...
   (let ()
     ...
     (test-case "Test authenticity for ECDSA signatures"
                (check-true (verify-ecdsa sig1 pub1 z1)))
     ...
     (test-case "# Test case 1.1: false signature r"
                (check-false (verify-ecdsa sig11 pub1 z1)))
     ...
     (test-case "# Test case 1.4: false pub key x"
                (check-false (verify-ecdsa sig1 pub1bis z1)))
     ...
     (test-case "# Test case 1.4bis: false pub key y"
                (check-false (verify-ecdsa sig1 pub1bisbis z1)))
     ...
     (test-case "# Test case 2: different signature and same pubkey"
                (check-true (verify-ecdsa sig2 pub1 z2)))

     (define e3 (generate-random)) ; a private key is just a random number
     (define pub3 (rmul-point G e3)) ; G*e to get the public key
     (define z3 (generate-random)) ; just a random message
     (define sig3 (sign-ecdsa e3 z3))

     (test-case "# Test case 3: sign and verify"
                (check-true (verify-ecdsa sig3 pub3 z3))))))
